...
def run_demo(net,action_net, image_provider, height_size, cpu):
    ...
    for img in image_provider:
        ...
        if i % 3 == 0:
            ...
            for pose in current_poses:
                ...
                if pose.pose_action == 'fall':
                    cv2.rectangle(img, (pose.bbox[0], pose.bbox[1]),
                                  (pose.bbox[0] + pose.bbox[2], pose.bbox[1] + pose.bbox[3]), (255, 0, 0),5)
                    cv2.putText(img, 'state: {}'.format(pose.pose_action), (pose.bbox[0], pose.bbox[1] - 16),
                                cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 0, 0), 3)
                    j += 1
                else:
                    cv2.rectangle(img, (pose.bbox[0], pose.bbox[1]),
                                  (pose.bbox[0] + pose.bbox[2], pose.bbox[1] + pose.bbox[3]), (0, 255, 0), 5)
                    cv2.putText(img, 'state: {}'.format(pose.pose_action), (pose.bbox[0], pose.bbox[1] - 16),
            ...
            if j>=1:
                cv2.imwrite(r'fall{}.jpg'.format(m), img)
                m += 1
            cv2.waitKey(1)
        i += 1
...
def detect_main(video_source = '',image_source = '',video_name=''):
    ...
    # *************************************************************************
    action_net = jit.load(r'.\weights\action.jit')
    # ************************************************************************
...
if __name__ == '__main__':
    ...
    detect_main(video_source=0,video_name='video1')
#    detect_main(video_source='../2.mp4')
